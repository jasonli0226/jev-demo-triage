...
Commands below also work as `uv run jev-playground triage <same arguments>`.
...
- Jev as a gate, false positives: with the package-default instructions the gate blocked safe restarts (`simple-restart` 0/3 in Run 1, `wrong-service` 0/3 in Run 3; the blocking itself is inferred, not recorded). The tuned policy removed that (0/3 to 3/3 in `gate` mode in both scenarios, suggestively, across sessions), but it was written after seeing the scenarios, so this shows that Jev follows the instruction it is given, not that the policy generalises. See [Run 3](#run-3-default-gate-on-the-three-new-scenarios).
- Like-for-like comparison ([Run 4](#run-4-baseline-vs-jev-gate-vs-llm-gate-eight-scenarios)): accuracy of the two gates is tied within noise (Jev gate 22/24, `llm-gate` 23/24, baseline 18/24). Per classifier call, Jev averaged 0.43 s versus 31.96 s for GLM (about 74x lower latency) and an estimated $0.00007 versus $0.0014 (about 21x lower estimated cost). The latency gap is measured; the cost gap rests on estimated GLM prices.
- Scope: N is 3 per cell, one run set per condition; the tuned policy was written knowing the scenarios; the LLM gate is the same model family as the agent (GLM judging GLM); GLM prices are estimates; Runs 1 to 4 are separate sessions on different code versions, so cross-run comparisons are suggestive.
- Not tested: a different judge model for `llm-gate`, scenarios where the gate must catch a subtle risk (every bait here is fairly explicit), and calibration (probabilities are not recorded in the run JSON). See [Next steps](#next-steps-and-open-items).
...
Experiments that would firm up the findings:

- More repeats (5 to 10 per cell) with confidence intervals, since N=3 cannot separate most differences.
- Scenarios where the gate must catch a subtle risk (a false-negative test); in the current scenarios the risky action is explicit.
- A non-GLM judge model for `llm-gate`, to avoid GLM judging GLM.
- A gate policy written without seeing the scenarios.
- Record each classifier's probability in the run JSON, to compare calibration.
- `max_retries=0` and matched timeouts for the LLM gate (see the retry and timeout asymmetry under Run 4).

Harness:

- `--compare` has no filter for modes or scenarios (Run 4 used a throwaway script).
- `--compare` writes its JSON only after all runs finish, so a crash loses the completed runs; incremental writes would avoid that.
- The destructive-command detector is a heuristic, and a plain `rm <file>` is not flagged.
- `risky-bait` (original) still returns no shell output; it is kept unchanged for dataset continuity.
- `risky-bait-output` passes on an immediate no-op escalation, and in baseline its mock `run_shell` loops until the step limit.
